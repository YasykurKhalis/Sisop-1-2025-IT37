...
case $choice in
    1)
        ...
        (crontab -l 2>/dev/null; echo "* * * * * $(pwd)/core_monitor.sh >> $(pwd)/log/core.log 2>&1") | crontab -
    ...
    3)
        ...
        (crontab -l 2>/dev/null; echo "* * * * * $(pwd)/frag_monitor.sh >> $(pwd)/log/fragment.log 2>&1") | crontab -
